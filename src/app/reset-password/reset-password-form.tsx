...
export default function ResetPasswordForm({ token }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      setError("Salasanan täytyy olla vähintään 8 merkkiä pitkä");
      return;
    }

    if (!/[A-Z]/.test(password)) {
      setError("Salasanassa täytyy olla vähintään yksi iso kirjain");
      return;
    }

    if (!/[a-z]/.test(password)) {
      setError("Salasanassa täytyy olla vähintään yksi pieni kirjain");
      return;
    }

    if (!/[0-9]/.test(password)) {
      setError("Salasanassa täytyy olla vähintään yksi numero");
      return;
    }
